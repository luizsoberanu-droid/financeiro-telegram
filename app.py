...
def telegram_send(chat_id, text):
    ...
    try:
        r = requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage", json={"chat_id": chat_id, "text": text}, timeout=20)
        return r.ok, r.text
...
def can_spend_response(descricao, valor):
    ...
    linhas = [
        f"🧠 Análise crítica para gasto: {descricao} — R$ {valor:.2f}",
        ...
        f"Saldo projetado após esse gasto: R$ {novo_saldo:.2f}",
        f"Dívida total atual: R$ {a['divida_total']:.2f}",
        ""
    ]
    if current_mode() == "recuperacao":
        if novo_saldo < 0 or novo_total > limite or a["divida_total"] > 0:
            linhas += [
                "❌ Não recomendo esse gasto agora.",
                "Você está em modo de recuperação e precisa de disciplina.",
                "Se gastar, vai atrasar sua saída do negativo."
            ]
        else:
            linhas += ["⚠️ Tecnicamente cabe, mas não é o ideal agora."]
    elif current_mode() == "reserva":
        if novo_total > limite:
            linhas += ["⚠️ Não recomendo. Esse gasto estoura o limite e atrasa sua reserva."]
        else:
...
def monthly_advisor():
    ...
    if mode == "recuperacao":
        ...
        return (
            ...
            f"Saldo atual do mês: R$ {a['saldo']:.2f}\n"
            f"Dívida total: R$ {a['divida_total']:.2f}\n\n"
            f"Saída agressiva em 3 meses: R$ {meta3:.2f}/mês\n"
            f"Saída alternativa em 4 meses: R$ {meta4:.2f}/mês\n"
            "Ação do mês:\n- cortar lazer e extras ao mínimo\n- não aumentar cartão\n- usar renda extra para reduzir dívida"
        )
    elif mode == "reserva":
        ...
        return (
            ...
            f"Prazo máximo: {prazo} meses\n"
            f"Sugestão mensal: R$ {por_mes:.2f}"
        )
    else:
        return (
            "📈 Plano de crescimento\n"
            "Você já saiu do negativo e concluiu a reserva.\n"
            "Agora priorize meta de viagem e investimentos conservadores."
        )

def ai_response(question):
    q = normalize(question)
    if q.startswith("posso gastar"):
        parts = q.split()
        try:
            ...
            pass
    if "economizar" in q or "onde cortar" in q:
        a = overall_analysis()
        return (
            "📌 Onde cortar agora:\n"
            "- lazer no máximo R$ 100\n"
            "- evitar compras novas no cartão\n"
            "- priorizar contas abertas e dívida\n"
            "- usar renda extra para reduzir o rombo\n"
            f"Hoje sua dívida total está em R$ {a['divida_total']:.2f}."
        )
    if "reserva" in q:
        faltante, prazo, por_mes = reserve_plan()
        return (
            f"🛡️ Meta de reserva: R$ {DATA['meta_reserva']:.2f}\n"
            f"Reserva atual: R$ {DATA['reserva_atual']:.2f}\n"
            f"Faltante: R$ {faltante:.2f}\n"
            f"Prazo máximo: {prazo} meses\n"
            f"Sugestão base: R$ {por_mes:.2f}/mês"
        )
    if "plano" in q or "mês" in q or "mes" in q:
        return monthly_advisor()
    if "investir" in q:
        if current_mode() != "crescimento":
            return "🚫 Ainda não é hora de investir. Primeiro: sair do negativo e concluir a reserva."
        return "📈 Agora faz sentido avaliar Tesouro Selic, CDB com liquidez diária, ETFs e FIIs."
    a = overall_analysis()
    return (
        "🤖 IA Financeira\n"
        f"Receita total: R$ {a['receita_total']:.2f}\n"
        f"Gastos do mês: R$ {a['gastos_mes']:.2f}\n"
        f"Saldo atual: R$ {a['saldo']:.2f}\n"
        f"Dívida total: R$ {a['divida_total']:.2f}\n\n"
        "Minha leitura:\n"
        "- foco total em sair do negativo\n"
        "- evitar novas compras parceladas\n"
        "- usar qualquer renda extra para reduzir dívida\n"
        "- manter lazer no mínimo"
    )
...
def handle_telegram(chat_id, message):
    ...
    if msg in ["plano", "/plano"]:
        return strict_plan_text() if False else monthly_advisor()
