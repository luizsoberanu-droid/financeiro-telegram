...
def telegram_webhook():
    ...
    try:
        ...
        if msg_lower.startswith(("quero comprar", "comprar ", "analisa compra de", "analisar compra de")):
            try:
                from services.wishlist_advisor_service import WishlistAdvisorService, limpar_query, buscar_preco_mercado_livre, classificar_prioridade
                produto = limpar_query(msg_lower)

                preco_info = buscar_preco_mercado_livre(produto)
                preco_estimado = preco_info.get("preco_mediano") if preco_info.get("ok") else None

                # Guarda o último item analisado para você poder responder:
                # "coloca na lista de desejos"
                s = session(chat_id)
                s["ultimo_desejo"] = {
                    "nome": produto,
                    "valor": preco_estimado,
                    "prioridade": classificar_prioridade(produto)
                }

                resposta = WishlistAdvisorService(db).analisar_compra(produto, preco_estimado)
                resposta += "\n\nSe quiser guardar para comprar depois, responda: colocar na lista de desejos."
                telegram_send(chat_id, resposta)
                return jsonify({"ok": True})
            except Exception as e:
                print(f"⚠️ Erro análise desejo: {e}")

        # Se a última compra analisada não coube, você pode mandar:
        # "coloca na lista de desejos"
        if msg_lower in [
            "coloca na lista de desejos",
            "colocar na lista de desejos",
            ...
            try:
                from models.database import Desejo
                ultimo = session(chat_id).get("ultimo_desejo")

                if not ultimo:
                    telegram_send(chat_id, "Não encontrei um item recente analisado. Me mande: quero comprar item.")
                    return jsonify({"ok": True})

                nome = ultimo.get("nome")
                valor = ultimo.get("valor") or 0
                prioridade = ultimo.get("prioridade") or "media"
                ...
                telegram_send(
                    chat_id,
                    f"✅ Adicionado à lista de desejos:\n"
                    f"Item: {nome}\n"
                    f"Valor estimado: R$ {float(valor):.2f}\n"
                    f"Prioridade: {prioridade.upper()}\n\n"
                    "Vou considerar esse item nas próximas análises antes de liberar gastos."
                )
                return jsonify({"ok": True})
            except Exception as e:
                telegram_send(chat_id, f"Tentei salvar na lista de desejos, mas deu erro: {e}")
                return jsonify({"ok": True})
