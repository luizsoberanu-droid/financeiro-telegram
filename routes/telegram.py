...
def fallback_financeiro(db, message):
    """Resposta de emergência para o Telegram nunca ficar mudo se LLM/API falhar."""
    try:
        ...
        if "gastar" in txt or "posso" in txt or "comprar" in txt:
            if dividas.get("total_divida", 0) > 0 or saldo.get("saldo_projetado", 0) <= 0:
                linhas += [
                    "Resposta direta: NÃO recomendo gastar agora.",
                    "Você ainda está em fase de recuperação. Cada gasto atrasa sua saída do negativo.",
                    "Próximo passo: segure o gasto e use qualquer renda extra para reduzir dívida."
                ]
            else:
                linhas += [
                    "Pode analisar com cautela.",
                    "Se não for essencial, eu seguraria para fortalecer sua reserva."
                ]
        elif "divida" in txt or "dívida" in txt or "negativo" in txt:
            meta = dividas.get("meta_3_meses", 0)
            linhas += [
                "Plano objetivo para zerar dívida:",
                f"- Meta agressiva: R$ {meta:.2f}/mês",
                "- cortar lazer ao mínimo",
                "- evitar novas parcelas",
                "- aplicar renda extra nas dívidas"
            ]
            ...
            try:
                ...
                linhas.append("Não consegui recalcular agora, mas a regra é: com dívida, cartão deve ficar em até 5% da renda total.")
        else:
            linhas += [
                "Orientação:",
                "- mantenha disciplina",
                ...
                "- priorize dívida antes de compras novas"
            ]

        return "\n".join(linhas)
    except Exception as e:
        return f"Estou ativo, mas tive erro ao consultar dados: {e}. Tente 'status' ou 'plano'."
...
def telegram_webhook():
    ...
    try:
        # Desejos por linguagem natural antes da IA
        ...
        # Tudo mais: IA com fallback para nunca ficar sem resposta
