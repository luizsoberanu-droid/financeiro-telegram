...
def api_relatorio_gerar():
    """Gera relatório e retorna status para o painel."""
    db = get_db_session()
    try:
        p = request.get_json() or {}
        mes_ref = p.get('mes') or datetime.now().strftime("%Y-%m")
        svc = PDFService(db)
        svc.gerar_relatorio_mensal(mes_ref)
        return jsonify({
            "ok": True,
            "mensagem": "Relatorio gerado com sucesso. Use o botao PDF para baixar.",
            "url": f"/api/relatorio/pdf?mes={mes_ref}"
        })
    finally:
        db.close()
