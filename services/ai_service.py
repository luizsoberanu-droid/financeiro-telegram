...
class FinancialTools:
    ...
    def _divida_bruta(self):
        from models.database import Divida
        return round(sum(d.valor for d in self.db.query(Divida).all()), 2)

    def _divida_ajustada(self, saldo_projetado):
        # Se o saldo projetado está positivo, ele reduz a dívida exibida.
        # Se o saldo projetado está negativo, ele aumenta a dívida exibida.
        return round(max(self._divida_bruta() - saldo_projetado, 0), 2)
    ...
    def get_saldo_atual(self):
        ...
        return {
            ...
            "divida_bruta": self._divida_bruta(),
            "divida_total": self._divida_ajustada(saldo),
    ...
    def get_analise_dividas(self):
        ...
        total_bruto = round(sum(d.valor for d in dividas), 2)
        saldo = self.get_saldo_atual()["saldo_projetado"]
        total_ajustado = round(max(total_bruto - saldo, 0), 2)
        detalhes = [{"nome": d.nome, "valor": round(d.valor, 2), "prioridade": d.ordem_prioridade} for d in dividas]

        return {
            "total_divida": total_ajustado,
            "divida_ajustada": total_ajustado,
            "divida_bruta": total_bruto,
            "saldo_usado_no_ajuste": round(saldo, 2),
            "detalhes": detalhes,
