...
class FinancialTools:
    ...
    def get_analise_dividas(self):
        ...
        return {
            "total_divida": round(total, 2),
            "detalhes": detalhes,
            "meta_3_meses": round(total / 3, 2) if total > 0 else 0,
            "meta_6_meses": round(total / 6, 2) if total > 0 else 0,
            "quantidade_credores": len(dividas),
            "status": "CRITICO" if total > 5000 else "ALTO" if total > 2000 else "MODERADO" if total > 0 else "LIMPO"
        }
